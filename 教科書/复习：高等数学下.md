...
3. **构造拉格朗日函数** 
   $$\begin{gathered}L=f(x,y,z)+\lambda g(x,y,z) \\\begin{cases}L_x=0 \\ L_y=0 \\ L_z=0 \\ L_\lambda=0\end{cases} \implies (x_0,y_0,z_0)\end{gathered}$$
   是为所求极值点。
...
## 傅里叶的结论
傅里叶级数收敛点：在$x=a$
$$\frac{\underset{x\to a^-}{\lim}f(x)+\underset{x\to a^+}{\lim}f(x)}{2}$$