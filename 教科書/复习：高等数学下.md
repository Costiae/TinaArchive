...
3. **对所求驻点判定**：$$AC-B^2\begin{cases}>0 & 有极值，且A \begin{cases}>0 & 极小值 \\ <0 & 极大值\end{cases}\\<0 & 无极值 \\=0 & 不确定\end{cases}$$
# 条件极值
1. 目标函数（题目的条件）$f(x,y,z)$
2. 条件函数（题目中量的限定条件）$g(x,y,z)$
3. **构造拉格朗日函数** $$\begin{align*}L=f(x,y,z)+\lambda g(x,y,z) \\\begin{cases}L_x=0 \\ L_y=0 \\ L_z=0 \\ L_z=0\end{cases} \implies (x_0,y_0,z_0)\end{align*}$$
   是为所求极值点。
...
另：$$\begin{gathered}A=a_1\hat{i}+b_1\hat{j}+c_1\hat{k} \ ;\ B=a_2\hat{i}+b_2\hat{j}+c_2\hat{k}\ ;\ C=a_3\hat{i}+b_3\hat{j}+c_3\hat{k}\\ (A \times B)\cdot C=\begin{vmatrix}a_1&b_1&c_1\\a_2&b_2&c_2\\a_3&b_3&c_3\end{vmatrix}\end{gathered}$$
